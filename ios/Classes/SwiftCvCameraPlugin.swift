...
public class SwiftCvCameraPlugin: NSObject, FlutterPlugin {
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let objectChangedEventChannel = FlutterEventChannel(name: "cv_camera/objectDetection", binaryMessenger: registrar.messenger())
        if #available(iOS 11.1, *) {
            let factory = FLNativeViewFactory(methodChannel: channel, eventChannel: eventChannel, objectChangedEventChannel: objectChangedEventChannel)
